...
export async function loadConfig(): Promise<Config> {
  ...
  try {
    ...
    if (!backendCanisterId && (!config.backend_canister_id || config.backend_canister_id === "undefined")) {
      console.warn("CANISTER_ID_BACKEND is not set, using dummy for local dev");
    ...
    const fullConfig = {
      ...
      backend_canister_id: (config.backend_canister_id === "undefined" || !config.backend_canister_id
        ? (backendCanisterId || "dummy-canister-id")
    ...
    console.warn("Falling back to dummy canister ID due to config load failure", err);
    const fallbackConfig = {
      backend_host: undefined,
      backend_canister_id: backendCanisterId || "dummy-canister-id",
